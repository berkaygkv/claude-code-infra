...
Append an idea to `{{cookiecutter.project_slug}}/inbox.md` mid-session.
...
Read `{{cookiecutter.project_slug}}/inbox.md` using native Read.
...
Use the current session number from `{{cookiecutter.project_slug}}/scratch.md` header (parse from "Session {N}").
